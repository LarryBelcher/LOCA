...
if __name__ == '__main__':
	...
	maskval = sys.argv[6]	#y or n
	
	if(maskval == 'y'): masked = 'mask'
	if(maskval == 'n'): masked = 'no-mask'
	...
	if(maskval == 'y'):
		...
		for shapedict,state in zip(m.world_info, m.world):
		    if shapedict['CNTR_ID'] not in ['US']: continue
		    poly = MplPolygon(state,facecolor='#a2a2a2',edgecolor='#a2a2a2',linestyle='solid',linewidth=2.001)
		    ax.add_patch(poly)
	...
	if(maskval == 'y'):
		#Add the final mask
		cmask_im = Image.open(cmask)
		m.imshow(cmask_im, origin='upper', alpha=1.0, zorder=10, aspect='auto', interpolation='nearest')
	...
	if(imgsize == 'HD' or imgsize =='HDSD'):
		if(var == 'tavg'):
			plt.text(xpt-30000, ypt-300000, 'Average temperature '+dec+'-'+decEnd, fontproperties=propr, size=15.7, color='#666666')
		if(var == 'tasmax'):
			plt.text(xpt-30000, ypt-300000, 'Average maximum temperature '+dec+'-'+decEnd, fontproperties=propr, size=15.7, color='#666666')
		if(var == 'tasmin'):
			plt.text(xpt-30000, ypt-300000, 'Average minimum temperature '+dec+'-'+decEnd, fontproperties=propr, size=15.7, color='#666666')
	

	isz = imgsize.lower()
	if(var == "tavg"):
		outpng = "../Examples/tavg/averagetemp-"+mms+"-"+dec+"-"+decEnd+"-LOCA-"+rcp+"_"+isz+'_'+masked+'.png'
	if(var == "tasmax"):
		outpng = "../Examples/tasmax/averagemaxtemp-"+mms+"-"+dec+"-"+decEnd+"-LOCA-"+rcp+"_"+isz+'_'+masked+'.png'
	if(var == "tasmin"):
		outpng = "../Examples/tasmin/averagemintemp-"+mms+"-"+dec+"-"+decEnd+"-LOCA-"+rcp+"_"+isz+'_'+masked+'.png'
